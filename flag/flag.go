...
// Same as Parse but handles flags anywhere in arguments.
func (f *FlagSet) ParseToEnd(arguments []string) error {
	err := f.Parse(arguments)
	if err != nil {
		return err
	}
	args := make([]string, 0)
	for i := len(arguments) - len(f.args) + 1; i < len(arguments); {
		if i > 1 && arguments[i-2] == "--" {
			break
		}
		args = append(args, f.args[0])
		err = f.Parse(arguments[i:])
		if err != nil {
			return err
		}
		i += 1 + len(arguments[i:]) - len(f.args)
	}
...
// Same as Parse but handles flags anywhere in arguments.
func ParseToEnd() {
	CommandLine.ParseToEnd(os.Args[1:])
}
